...
bool PhysicsModule::Initialize()
{
	...
	for (auto& ptr : m_Colliders) {
		reactphysics3d::CollisionBody* collisionBody  = ptr->initialize_React3D(m_World);
		m_ColliderMap.insert(std::make_pair(collisionBody, ptr.get()));
	}
