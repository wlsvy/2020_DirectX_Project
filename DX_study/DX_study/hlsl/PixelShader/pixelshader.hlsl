...
{
    //input.inNormal = normalize(input.inNormal); //정점 상의 노말 벡터를 보간하면, 다시 정규화를 해줘야 할 필요가 있음. 
    ...
    //float attenuationFactor = 1 / dot(float3(dynamiLightAttenuation_a, dynamiLightAttenuation_b, dynamiLightAttenuation_c), float3(1, distanceToLight, distanceToLight * distanceToLight));
    ...
    //finalColor = input.inNormal * appliedLight;
